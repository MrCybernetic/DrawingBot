/**
 ...
 Gcode Stipple Generator, v. 1.0
 Original Copyright (C) 2013 by Windell H. Oskay, www.evilmadscientist.com
 Modified by Cybernetic to generate gcode instead of SVG, https://github.com/MrCybernetic
 ...
 *******************************************************************************
 ...
 v 1.0 Cybernetic forked from 2.31
 * Modify to work with new ControlP5 library
 * Modify the output file, from SVG to simplifyGCode
 * Modify to work with Processing 3.5.3
...
void setup()
{
  ...
  smooth();
  noStroke();
  fill(90,90,150); // Background fill color, for control section
  ...
  cp5.addButton("SAVE_PATH", 10, 25, GUIFudge + 4*GuiRowSpacing, 160, 10); 
  cp5.getController("SAVE_PATH").setCaptionLabel("Save \"TSP\" Path (.GCODE format)");
...
void SavefileSelected(File selection) {
  if (selection == null) {
    ...
    savePath = selection.getAbsolutePath();
      ...
      if ( p[p.length - 1].equals("gcode"))
        fileOK = true;
      if ( p[p.length - 1].equals("gcode"))
        fileOK = true;      

      if (fileOK == false)
        savePath = savePath + ".gcode";
...
void SAVE_SVG(float theValue) {  
  ...
  if (pausemode != true) {
    ...
    selectOutput("Output .gcode file name:", "SavefileSelected");
...
void draw()
{
  ...
  noStroke();
  fill(90,90,110);   // Background fill color
  ...
  text("CyberStippleGen Modified by Cybernetic      (v. 1.0)", TextColumnStart, mainheight + 15);
  text("Original made by Evil Mad Scientist Laboratories", TextColumnStart, mainheight + 30);
  text("www.evilmadscientist.com/go/stipple2", TextColumnStart, mainheight + 45);
  ...
  if (SaveNow > 0) {
    ...
    StatusDisplay = "Saving GCODE File";
    SaveNow = 0;
    ...
    int xOffset = (int) (300 - (SVGscale * mainwidth / 2));
    int yOffset = (int) (300 - (SVGscale * mainheight / 2));
    ...
    { // Plot the PATH between the points only.
      ...
      println("Save Stipple File (GCODE)");
    ...
    FileModeTSP = false; // reset for next time
      ...
      if (FileModeTSP) 
      ErrorDisplay = "TSP Path .GCODE file Saved";
    else
      ErrorDisplay = "Stipple .GCODE file saved ";
